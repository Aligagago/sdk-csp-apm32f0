...
int uart_init(void)
{
    ...
    for (i = 0; i < sizeof(usart_config) / sizeof(struct apm32_usart); i++)
    {
        ...
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_9, GPIO_AF_PIN1);
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_10, GPIO_AF_PIN1);
        USART_Config(USART1, &USART_ConfigStruct);
        USART_Enable(USART1);
#endif
#ifdef BSP_USING_UART2
        RCM_EnableAPB1PeriphClock(RCM_APB1_PERIPH_USART2);
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_2, GPIO_AF_PIN1);
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_3, GPIO_AF_PIN1);
