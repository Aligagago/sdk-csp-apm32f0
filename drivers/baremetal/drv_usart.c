...
int uart_init(void)
{
    ...
    for (i = 0; i < sizeof(usart_config) / sizeof(struct apm32_usart); i++)
    {
        ...
        RCM_EnableAHBPeriphClock(RCM_AHB_PERIPH_GPIOA << (_uart_port_get(usart_config[i].rx_pin_name) - 'A'));
        ...
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_9, GPIO_AF_USART1);
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_10, GPIO_AF_USART1);
        USART_Config(USART1, &USART_ConfigStruct);
        USART_Enable(USART1);
#endif
#ifdef BSP_USING_UART2
        RCM_EnableAPB1PeriphClock(RCM_APB1_PERIPH_USART2);
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_2, GPIO_AF_USART2);
        GPIO_ConfigPinAF(gpio_port, GPIO_PIN_SOURCE_3, GPIO_AF_USART2);
